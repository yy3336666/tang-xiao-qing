from keras.layers import Input, Dense, LSTM, Conv1D, Dropout, Bidirectional, Multiply,TimeDistributed
...
def attention_model(INPUT_DIMS = 13,TIME_STEPS = 20,lstm_units = 64):
    ...
    return model
def cnn_lstm_model(X_train,yuan_X_train):
    model = Sequential()
    model.add(Conv1D(filters=16, kernel_size=1, activation='relu',input_shape=(X_train.shape[0],1)))
    model.add(Dropout(0.3))
    model.add(Bidirectional(LSTM(50, activation='relu',name='bilstm')))
    model.add((Flatten()))
    model.add(Dense(1))
    yuan_model = Sequential()
    yuan_model.add(Conv1D(filters=64, kernel_size=1, activation='relu',input_shape=(X_train.shape[0],5)))
    yuan_model.add(Dropout(0.3))
    yuan_model.add(Bidirectional(LSTM(50, activation='relu',name='bilstm')))
    yuan_model.add(Flatten())
