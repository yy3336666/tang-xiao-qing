...
def train(args, train_dataset, model, tokenizer):
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)):
        ...
        '''enumerate()函数是枚举列举的意思，参数为可遍历/可迭代的对象(如列表、字符串)，多用于在for循环中得到计数，利用它可以同时获得索引和值，即需要index和value值的时候可以使用enumerate'''
...
def main():
    ...
    '''CPU or GPU'''
    # Setup CUDA, GPU & distributed training分布式训练
    if args.local_rank == -1 or args.no_cuda:
        ...
        args.n_gpu = 1
    args.device = device
    '''n_gpu=8 per_gpu_train/eval_batch_size=train/eval_batch_size(128)//8'''
    ...
    '''Process rank: -1, device: cuda, n_gpu: 8, distributed training: False, 16-bits training: False'''
    ...
    '''model_type为roberta--RobertaConfig, RobertaForSequenceClassification, RobertaTokenizer'''
    ...
    if args.model == "devign":
        ...
        print('devign model does not exists')
        '''
